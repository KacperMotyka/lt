/**
 * @文件名称: NbrCrsFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-13 上午10:37:23
 * @版本:     1.0.0
 */
...
/**
 * @类功能说明: 邻号乘积和过滤,相邻两个号码的乘积之和
 * @类修改者:     
 * @修改日期:   
 * @修改说明:   
 * @作者:       robin
 * @创建时间:   2011-12-13 上午10:37:23
 * @版本:       1.0.0
 */

public class NbrCrsFilterTask extends Task {

	private static final Logger log = Logger.getLogger(NbrCrsFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public NbrCrsFilterTask() {
		least = 0;
		most = 117;
		this.fatal = false;
	}

	public NbrCrsFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int crsSum = SystemFunctions.getNbrCrsSum(bet);
				if (crsSum > most || crsSum < least) {
					vit.remove();
					drop++;
				}
			}
