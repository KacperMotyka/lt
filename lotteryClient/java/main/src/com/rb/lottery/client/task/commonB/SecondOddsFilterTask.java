/**
 * @文件名称: SecondOddsFilterTask.java
 * @类路径:   com.rb.lottery.task.commonB
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-13 下午02:11:42
 * @版本:     1.0.0
 */
 ...
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-13 下午02:11:42
 * @版本: 1.0.0
 */

public class SecondOddsFilterTask extends Task {

	private static final Logger log = Logger
			.getLogger(SecondOddsFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public SecondOddsFilterTask() {
		least = 0;
		most = 14;
		this.fatal = false;
	}

	public SecondOddsFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			String secondBet = SystemProcessor.generateSpeOdds(2, key);
			...
			while (vit.hasNext()) {
				...
				int fbc = SystemFunctions.getMatchingCount(secondBet, bet);
				if (fbc > most || fbc < least) {
					vit.remove();
					drop++;
				}
			}
