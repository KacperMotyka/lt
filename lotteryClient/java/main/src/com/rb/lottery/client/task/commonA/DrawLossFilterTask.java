/**
 * @文件名称: DrawLossFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-13 上午09:59:35
 * @版本:     1.0.0
 */
...
/**
 * @类功能说明:平负连场过滤 单注中 "3"和"1" 任意组合在一起的最大连续场次数（在该连续场次中必须包含"3"和"1"）
 * @类修改者:
 * @修改日期:
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-13 上午09:59:35
 * @版本: 1.0.0
 */

public class DrawLossFilterTask extends Task {

	private static final Logger log = Logger.getLogger(DrawLossFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public DrawLossFilterTask() {
		least = 0;
		most = 14;
		this.fatal = false;
	}

	public DrawLossFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int count = SystemFunctions.getCombConCount(bet,
						SystemConstants.ONE, SystemConstants.ONE);
