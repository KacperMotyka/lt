/**
 * @文件名称: WinLossFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-13 上午09:57:09
 * @版本:     1.0.0
 */
...
/**
 * @类功能说明: 胜负连场过滤 单注中 "3"和"0" 任意组合在一起的最大连续场次数（在该连续场次中必须包含"3"和"0"）
 * @类修改者:
 * @修改日期:
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-13 上午09:57:09
 * @版本: 1.0.0
 */

public class WinLossFilterTask extends Task {

	private static final Logger log = Logger.getLogger(WinLossFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public WinLossFilterTask() {
		least = 0;
		most = 14;
		this.fatal = false;
	}

	public WinLossFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int count = SystemFunctions.getCombConCount(bet,
						SystemConstants.THREE, SystemConstants.ZERO);
