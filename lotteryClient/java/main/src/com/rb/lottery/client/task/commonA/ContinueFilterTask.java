/**
 * @文件名称: ContinueFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-12 下午04:38:08
 * @版本:     1.0.0
 */
...
/**
 * @类功能说明: 连场数过滤,投注结果中相邻两个号码相同就是一个连号,如：33、11、00
 * @类修改者:
 * @修改日期:
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-12 下午04:38:08
 * @版本: 1.0.0
 */

public class ContinueFilterTask extends Task {

	private static final Logger log = Logger
			.getLogger(ContinueFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public ContinueFilterTask() {
		least = 0;
		most = 14;
		this.fatal = false;
	}

	public ContinueFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int con = SystemFunctions.getContinueCount(bet);
				if (con > most || con < least) {
					vit.remove();
					drop++;
				}
			}
