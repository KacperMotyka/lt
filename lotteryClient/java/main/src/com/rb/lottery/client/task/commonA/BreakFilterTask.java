/**
 * @文件名称: BreakFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-12 下午01:05:02
 * @版本:     1.0.0
 */
 ...
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-12 下午01:05:02
 * @版本: 1.0.0
 */

public class BreakFilterTask extends Task {

	private static final Logger log = Logger.getLogger(BreakFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public BreakFilterTask() {
		least = 0;
		most = 14;
		this.fatal = false;
	}

	public BreakFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int breaks = SystemFunctions.getBreakCount(bet);
				if (breaks > most || breaks < least) {
					vit.remove();
					drop++;
				}
			}
