/**
 * @文件名称: PosCrsFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-13 上午10:49:19
 * @版本:     1.0.0
 */
...
/**
 * @类功能说明: 号码位积和过滤,比赛的3、1、0结果与其序号的乘积之和
 * @类修改者:
 * @修改日期:
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-13 上午10:49:19
 * @版本: 1.0.0
 */

public class PosCrsFilterTask extends Task {

	private static final Logger log = Logger.getLogger(PosCrsFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public PosCrsFilterTask() {
		least = 0;
		most = 273;
		this.fatal = false;
	}

	public PosCrsFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int posCrsSum = SystemFunctions.getPosCrsSum(bet);
				if (posCrsSum > most || posCrsSum < least) {
					vit.remove();
					drop++;
				}
			}
