/**
 * @文件名称: LossConFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-13 上午09:02:29
 * @版本:     1.0.0
 */
 ...
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-13 上午09:02:29
 * @版本: 1.0.0
 */

public class LossConFilterTask extends Task {

	private static final Logger log = Logger.getLogger(LossConFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public LossConFilterTask() {
		least = 0;
		most = 14;
		this.fatal = false;
	}

	public LossConFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int con0 = SystemFunctions.getMxContinueCount(bet, '0');
				if (con0 > most || con0 < least) {
					vit.remove();
					drop++;
				}
			}
