/**
 * @文件名称: DrawConFilterTask.java
 * @类路径:   com.rb.lottery.task.commonA
 * @描述:     TODO
 * @作者:     robin
 * @时间:     2011-12-13 上午09:00:37
 * @版本:     1.0.0
 */
 ...
 * @修改说明:
 * @作者: robin
 * @创建时间: 2011-12-13 上午09:00:37
 * @版本: 1.0.0
 */

public class DrawConFilterTask extends Task {

	private static final Logger log = Logger.getLogger(DrawConFilterTask.class);

	private float least;
	private float most;
	private boolean fatal;

	public DrawConFilterTask() {
		least = 0;
		most = 14;
		this.fatal = false;
	}

	public DrawConFilterTask(float least, float most, boolean ft) {
		this.least = least;
		this.most = most;
		this.fatal = ft;
	}
	...
	public void run() {
		...
		while (it.hasNext()) {
			...
			while (vit.hasNext()) {
				...
				int con1 = SystemFunctions.getMxContinueCount(bet, '1');
				if (con1 > most || con1 < least) {
					vit.remove();
					drop++;
				}
			}
